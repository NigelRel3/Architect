...
class Workspace extends MySQL	{
	...
	protected static $fields = ["id", "Name", "UserID", "CreatedOn", "Config", "Windows"];
    ...
    protected function formatToDB(): array  {
		...
		if ( $output["Windows"] )	{
			$output["Windows"] = json_encode($output["Windows"]);
		}
		// FormatToDBEnd
        return $output;
    }
    ...
    protected function formatFromDB(array &$row): void  {
		...
		if ( !empty($row["Windows"]) )	{
			$row["Windows"] = json_decode($row["Windows"], self::$jsonArrayMode);
		}
		// FormatFromDBEnd
    }
