...
class Menu extends MySQL	{
    ...
    protected function buildSubMenus ( $menuID, $menuItems )	{
    	...
    	foreach ( $menuItems as $subItem )	{
    		if ( $subItem->parentMenuID == $menuID )	{
    			...
    			if ( !empty($subItem->addChildData) )	{
    				$jsonMenu['data']['MenuData']['addChildData'] = $subItem->addChildData;
    			}
    			if ( $jsonMenu['folder'] )	{
    ...
    public function fetchByKey ( int $workspaceID, int $key )	{
    	$menus = $this->fetchWhere ( "WorkspaceID = :WorkspaceID and `key` = :key",
    			["WorkspaceID" => $workspaceID, 'key' => $key]);
    	if ( count($menus) == 1 )	{
    		$this->data = $menus[0]->data;
    		$this->active = true;
    	}
    	else	{
    		$this->data = [];
