    ...
    public function loginData ( Request $request, Response $response  )   {
    	...
    	if ( $this->user->fetch([$id]) )	{
	    	...
	    	foreach ( $wsData as &$ws )	{
	    		// Fetch menu for workspace
	    		$menus = $this->menu->getWorkspaceMenu($ws['id']);
	    		$ws['Menu'] = $menus;
// 	    		$ws['Config'] = $this->processPanes($ws['Config'], $menus, $paneID);
	    		$ws['Config'] = $ws['Config'];
	    		$windows = [];
	    		foreach ($ws['Windows'] as $window )	{
	    			$windows[] = $this->processPanes($window, $menus, $paneID);
	    		}
	    		$ws['Windows'] = $windows;
    ...
    protected function processPanes ( $panes, $menus, int &$paneID )	{
    	$newTabs = $panes;
    	$openToPanelSet = false;

    	if ( isset($panes['tabs'] ))	{
     		unset($newTabs['tabs']);
    		foreach ( $panes['tabs']  as $tab)	{
    			$openToThisPanel = false;
    			if ( is_array($tab) )	{
    				$openToThisPanel = $tab['openToThisPanel'] ?? false;
    				$openToPanelSet = $openToPanelSet ?: $openToThisPanel;
    				$tab = $tab['id'];
    			}
	    		if ( $tab === 0 )	{
	    			$newTabs['tabs'][$tab] = ['ComponentID' => 1,
	    				'Title' => 'Main Menu',
	    				'PaneID' => $paneID++
	    			];
	    			if ( $openToThisPanel )	{
	    				$newTabs['tabs'][$tab]['OpenToThisPanel'] = $openToThisPanel;
	    			}

	    		}
    			else	{
    				// Translate tab into menu key data
	    			$keyNode = $this->menu->extractMenu($tab, $menus);
	    			if ( $keyNode )	{
	    				$newTabs['tabs'][$tab] = [
    		...
    		$out = [];
    		foreach ( $panes['panes'] ?? $panes  as $pane )	{
    			$addPanes = $this->processPanes($pane, $menus, $paneID);
    			$out[] = $addPanes;
    			$out['OpenToThisPanel'] = $addPanes['OpenToThisPanel'] ?? false;
    		}
    		if ( isset($panes['panes'] ))	{
    			$newTabs['panes'] = $out;
    		}
    		else 	{
    			$newTabs = $out;
    	...
    	if ( $openToPanelSet )	{
    		$newTabs['OpenToThisPanel'] = $openToPanelSet;
    	}
    	return $newTabs;
    }
    ...
    public function save (Request $request, Response $response  )   {
    	...
    	foreach ( $newConfig['panes'] as $id => $pane )	{
    		// If all tabs are closed, don't save pane
    		if ( isset($pane['tabs']) )	{
    			$newConfig['panes'][$id]['tabs'] = array_keys($pane['tabs']);
    		}
    		else	{
    			unset($newConfig['panes'][$id]);
    		}
    	}
    	$this->workspace->Config = $newConfig;
