...
class StatsLoad extends MySQL	{
	...
	protected static $fields = ["id", "Name", "CreatedOn", "Notes", "OwnerID", "DataSource", "DataStartPoint", "DataEndPoint", "config", "ImportType", "Group", "GroupKey"];
    ...
    protected function formatFromDB(array &$row): void  {
		...
		if ( !is_null($row["GroupKey"]) )	{
			$row["GroupKey"] = intval($row["GroupKey"]);
		}
		// FormatFromDBEnd
    }
}
